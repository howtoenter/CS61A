...
def subseqs(s):
    ...
    if s == []:  #base case
        ...
        subset = subseqs(s[1:])       #without the first element
        return subset + insert_into_all(s[0], subset)   #case with the first element and without the first element
...
def trade(first, second):
    ...
    equal_prefix = lambda: sum(first[:m]) == sum(second[:n])#lambda 之后要加()!
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    score_sofar, total_socre_sofar = 0, 0
    def glookup(key, score):
        nonlocal score_sofar, total_socre_sofar
        score_sofar += score
        total_socre_sofar += class_assignments[key]
        return score_sofar / total_socre_sofar
    return glookup
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 or n == 2:
        ...
        return num_trees(n - 1)*(2*n - 3)*2//n
