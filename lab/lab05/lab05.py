...
def sprout_leaves(t, leaves):
    ...
    """
    if is_leaf(t):
      return tree(t, [tree(i) for i in leaves])
    
    for son in branches(t):
      if is_leaf(son):
        tree(son, [tree(i) for i in leaves])
      else:
        return sprout_leaves(son, leaves)
    return t
    """
    if is_leaf(t):
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        pad_t1 = fewer_branch_t + [tree(0) for branch in range(len(more_branch_t) - len(fewer_branch_t))]
