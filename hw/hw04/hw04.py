...
def arm(length, mobile_or_planet):
    ...
    assert is_mobile(mobile_or_planet) or is_planet(mobile_or_planet), "must be a mobile or planet"
...
def quadratic(x, a, b, c):
    ...
    if a > 0:
        if lower_bound(x) < point < upper_bound(x):
            return interval(f(point), max(f(lower_bound(x)), f(upper_bound(x))))
        elif point < lower_bound(x):
            return interval(f(lower_bound(x)), f(upper_bound(x)))
        elif point > upper_bound(x):
            return interval(f(upper_bound(x)), lower_bound(x))
    elif a < 0:
        if lower_bound(x) < point < upper_bound(x):
            return interval(min(f(lower_bound(x)), f(upper_bound(x))), f(point))
        elif point > upper_bound(x):
            return interval(f(lower_bound(x)), f(upper_bound(x)))
        elif point < lower_bound(x):
            return interval(f(upper_bound(x)), lower_bound(x))
    ...
    f1, f2, m = f(x1), f(x2), f(point)
    if x1 <= point <= x2:
        if a > 0 :
            return interval(m, max(f1, f2))
        else:
            return interval(min(f1, f2), m)
    else:
