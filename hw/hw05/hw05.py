...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, word):
        ...
        if locked:
            return f"Too many incorrect attempts. Attempts: {temp}"
        if word != password:
            temp.append(word)
            if len(temp) == 3:
                locked = True
...
def remainders_generator(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    #yield elements with a particular remainder
    def inner(n):#generator function
        while True:#不理解
            for i in naturals():
                yield i*m + n
    yield inner(0)
    for j in range(-m+1, 0):
        yield inner(j)
    #yield each successive generator function with remainder 0 ~ (m-1)
    #think about some kind of while loop or for loop
